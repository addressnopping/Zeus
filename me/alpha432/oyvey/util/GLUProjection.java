...
public final class GLUProjection {
  ...
  public void updateMatrices(IntBuffer viewport, FloatBuffer modelview, FloatBuffer projection, double widthScale, double heightScale) {
    ...
    if (fv.x < 0.0D)
      yaw = 360.0D - yaw; 
    double pitch = 0.0D;
    pitch = ((-fv.y > 0.0D && yaw >= 90.0D && yaw < 270.0D) || (fv.y > 0.0D && (yaw < 90.0D || yaw >= 270.0D))) ? Math.toDegrees(Math.atan2(nuv.cross(uv).length(), nuv.dot(uv))) : -Math.toDegrees(Math.atan2(nuv.cross(uv).length(), nuv.dot(uv)));
  ...
  public Projection project(double x, double y, double z, ClampMode clampModeOutside, boolean extrudeInverted) {
    ...
    boolean outsideFrustum = (frustum[0] || frustum[1] || frustum[2] || frustum[3]), bl = outsideFrustum;
    if (outsideFrustum) {
      ...
      boolean outsideInvertedFrustum = (invFrustum[0] || invFrustum[1] || invFrustum[2] || invFrustum[3]), bl2 = outsideInvertedFrustum;
      if ((extrudeInverted && !outsideInvertedFrustum) || (outsideInvertedFrustum && clampModeOutside != ClampMode.NONE)) {
        ...
        if (!GLU.gluProject((float)x, (float)y, (float)z, this.modelview, this.projection, this.viewport, this.coords))
          return new Projection(0.0D, 0.0D, Projection.Type.FAIL); 
        double d3 = this.coords.get(0) * this.widthScale;
        double d4 = (this.displayHeight - this.coords.get(1)) * this.heightScale;
        if (opposite) {
          d3 = this.displayWidth * this.widthScale - d3;
          d4 = this.displayHeight * this.heightScale - d4;
        } 
        if (d3 < 0.0D) {
          d3 = 0.0D;
        } else if (d3 > this.displayWidth * this.widthScale) {
          d3 = this.displayWidth * this.widthScale;
        } 
        if (d4 < 0.0D) {
          d4 = 0.0D;
          return new Projection(d3, d4, outsideInvertedFrustum ? Projection.Type.OUTSIDE : Projection.Type.INVERTED);
        } 
        if (d4 <= this.displayHeight * this.heightScale)
          return new Projection(d3, d4, outsideInvertedFrustum ? Projection.Type.OUTSIDE : Projection.Type.INVERTED); 
        d4 = this.displayHeight * this.heightScale;
        return new Projection(d3, d4, outsideInvertedFrustum ? Projection.Type.OUTSIDE : Projection.Type.INVERTED);
      } 
      if (!GLU.gluProject((float)x, (float)y, (float)z, this.modelview, this.projection, this.viewport, this.coords))
        return new Projection(0.0D, 0.0D, Projection.Type.FAIL); 
      ...
      if (!opposite)
        return new Projection(d1, d2, outsideInvertedFrustum ? Projection.Type.OUTSIDE : Projection.Type.INVERTED); 
