...
class MethodWriter extends MethodVisitor {
  ...
  public void visitParameter(String name, int access) {
    ...
    this.methodParameters.putShort((name == null) ? 0 : this.cw.newUTF8(name))
      .putShort(access);
  }
    ...
    if (this.compute == 1) {
      if (this.currentBlock.frame == null) {
        ...
        this.currentBlock.frame.initInputFrame(this.cw, this.access, 
            Type.getArgumentTypes(this.descriptor), nLocal);
      ...
      for (i = 0; i < nLocal; i++) {
        if (local[i] instanceof String) {
          this.frame[frameIndex++] = 0x1700000 | this.cw
            .addType((String)local[i]);
        } else if (local[i] instanceof Integer) {
          this.frame[frameIndex++] = ((Integer)local[i]).intValue();
        } else {
          this.frame[frameIndex++] = 0x1800000 | this.cw
            .addUninitializedType("", ((Label)local[i]).position);
        } 
      } 
      for (i = 0; i < nStack; i++) {
        if (stack[i] instanceof String) {
          this.frame[frameIndex++] = 0x1700000 | this.cw
            .addType((String)stack[i]);
        } else if (stack[i] instanceof Integer) {
          this.frame[frameIndex++] = ((Integer)stack[i]).intValue();
        } else {
          this.frame[frameIndex++] = 0x1800000 | this.cw
            .addUninitializedType("", ((Label)stack[i]).position);
        } 
      } 
      endFrame();
    } else {
    ...
    if (this.compute == 0) {
      ...
      while (changed != null) {
        ...
        if ((label.status & 0x10) != 0)
          label.status |= 0x20; 
        label.status |= 0x40;
        int blockMax = f.inputStack.length + label.outputStackMax;
        if (blockMax > max)
          max = blockMax; 
        ...
        while (l != null) {
          if ((l.status & 0x80) != 0) {
            ...
            if ((subroutine.status & 0x400) == 0) {
              id++;
              subroutine.visitSubroutine(null, id / 32L << 32L | 1L << id % 32, this.subroutines);
            } 
          } 
          l = l.successor;
        } 
    ...
    while (true) {
      ...
      switch (this.descriptor.charAt(i++)) {
        ...
        case '[':
          ...
          this.frame[frameIndex++] = 0x1700000 | this.cw
            .addType(this.descriptor.substring(j, ++i));
          continue;
        case 'L':
          while (this.descriptor.charAt(i) != ';')
            i++; 
          this.frame[frameIndex++] = 0x1700000 | this.cw
            .addType(this.descriptor.substring(j + 1, i++));
          continue;
      } 
      break;
    } 
  ...
  private void writeFrame() {
    int delta, clocalsSize = this.frame[1];
        ...
        switch (t & 0xFF00000) {
          case 24117248:
            this.stackMap.putByte(7).putShort(this.cw
                .newClass((this.cw.typeTable[v]).strVal1));
            break;
          case 25165824:
