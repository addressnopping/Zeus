...
public final class MethodSlice {
  ...
  private MethodSlice(ISliceContext owner, String id, InjectionPoint from, InjectionPoint to) {
    if (from == null && to == null)
      throw new InvalidSliceException(owner, String.format("%s is redundant. No 'from' or 'to' value specified", new Object[] { this })); 
    this.owner = owner;
  ...
  public ReadOnlyInsnList getSlice(MethodNode method) {
    ...
    int end = find(method, this.to, max, start, this.name + "(to)");
    if (start > end)
      throw new InvalidSliceException(this.owner, String.format("%s is negative size. Range(%d -> %d)", new Object[] { describe(), Integer.valueOf(start), Integer.valueOf(end) })); 
  ...
  private int find(MethodNode method, InjectionPoint injectionPoint, int defaultValue, int failValue, String description) {
    ...
    if (nodes.size() != 1 && select == InjectionPoint.Selector.ONE)
      throw new InvalidSliceException(this.owner, String.format("%s requires 1 result but found %d", new Object[] { describe(description), Integer.valueOf(nodes.size()) })); 
    if (!result) {
  ...
  private static String describeSlice(String description, ISliceContext owner) {
    ...
    return String.format("%s->%s(%s)::%s%s", new Object[] { owner.getContext(), annotation, description, method.name, method.desc });
  }
  
  private static String getSliceName(String id) {
    return String.format("@Slice[%s]", new Object[] { Strings.nullToEmpty(id) });
  }
