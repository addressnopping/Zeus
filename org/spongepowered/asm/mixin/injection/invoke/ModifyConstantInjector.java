...
public class ModifyConstantInjector extends RedirectInjector {
  ...
  private void injectConstantModifier(Target target, AbstractInsnNode constNode) {
    ...
    if (constantType.getSort() <= 5 && this.info.getContext().getOption(MixinEnvironment.Option.DEBUG_VERBOSE))
      checkNarrowing(target, constNode, constantType); 
  ...
  private void checkNarrowing(Target target, AbstractInsnNode constNode, Type constantType) {
    ...
    if (pop instanceof FieldInsnNode) {
      ...
      checkNarrowing(target, constNode, constantType, fieldType, target.indexOf(pop), String.format("%s %s %s.%s", new Object[] { Bytecode.getOpcodeName(pop), SignaturePrinter.getTypeName(fieldType, false), fieldNode.owner.replace('/', '.'), fieldNode.name }));
    } else if (pop.getOpcode() == 172) {
      checkNarrowing(target, constNode, constantType, target.returnType, target.indexOf(pop), "RETURN " + 
      ...
      if (localVar != null && localVar.desc != null) {
        ...
        checkNarrowing(target, constNode, constantType, localType, target.indexOf(pop), String.format("ISTORE[var=%d] %s %s", new Object[] { Integer.valueOf(var), 
  ...
  private void checkNarrowing(Target target, AbstractInsnNode constNode, Type constantType, Type type, int index, String description) {
    ...
    if (toSort < fromSort) {
      ...
      Injector.logger.log(level, "Narrowing conversion of <{}> to <{}> in {} target {} at opcode {} ({}){}", new Object[] { fromType, toType, this.info, target, 
            Integer.valueOf(index), description, message });
    } 
  }
}
