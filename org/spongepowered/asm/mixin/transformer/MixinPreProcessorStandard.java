...
class MixinPreProcessorStandard {
  ...
  protected void prepareShadow(MixinInfo.MixinMethodNode mixinMethod, ClassInfo.Method method) {
    ...
    if (mixinMethod.name.startsWith(prefix)) {
      Annotations.setVisible(mixinMethod, MixinRenamed.class, new Object[] { "originalName", mixinMethod.name });
  ...
  protected void attachMethods(MixinTargetContext context) {
    for (Iterator<MixinInfo.MixinMethodNode> iter = this.classNode.mixinMethods.iterator(); iter.hasNext(); ) {
      MixinInfo.MixinMethodNode mixinMethod = iter.next();
      if (!validateMethod(context, mixinMethod)) {
        iter.remove();
        continue;
      } 
      if (attachInjectorMethod(context, mixinMethod)) {
        context.addMixinMethod(mixinMethod);
        continue;
      } 
      if (attachAccessorMethod(context, mixinMethod)) {
        iter.remove();
        continue;
      } 
      if (attachShadowMethod(context, mixinMethod)) {
        context.addShadowMethod(mixinMethod);
        iter.remove();
        continue;
      } 
      if (attachOverwriteMethod(context, mixinMethod)) {
        context.addMixinMethod(mixinMethod);
        continue;
      } 
      if (attachUniqueMethod(context, mixinMethod)) {
        iter.remove();
        continue;
      } 
  ...
  protected boolean attachAccessorMethod(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod) {
    return (attachAccessorMethod(context, mixinMethod, SpecialMethod.ACCESSOR) || 
      attachAccessorMethod(context, mixinMethod, SpecialMethod.INVOKER));
  }
    ...
    if (target == null) {
      ...
      if (target == null)
        throw new InvalidMixinException(this.mixin, 
            String.format("%s method %s in %s was not located in the target class %s. %s%s", new Object[] { type, mixinMethod.name, this.mixin, context.getTarget(), context.getReferenceMapper().getStatus(), 
                getDynamicInfo(mixinMethod) })); 
      mixinMethod.name = method.renameTo(target.name);
    } 
    if ("<init>".equals(target.name))
      throw new InvalidMixinException(this.mixin, String.format("Nice try! %s in %s cannot alias a constructor", new Object[] { mixinMethod.name, this.mixin })); 
    if (!Bytecode.compareFlags(mixinMethod, target, 8))
      throw new InvalidMixinException(this.mixin, String.format("STATIC modifier of %s method %s in %s does not match the target", new Object[] { type, mixinMethod.name, this.mixin })); 
    conformVisibility(context, mixinMethod, type, target);
    if (!target.name.equals(mixinMethod.name)) {
      if (type.isOverwrite && (target.access & 0x2) == 0)
  ...
  protected void checkMethodNotUnique(ClassInfo.Method method, SpecialMethod type) {
    if (method.isUnique())
      throw new InvalidMixinException(this.mixin, String.format("%s method %s in %s cannot be @Unique", new Object[] { type, method.getName(), this.mixin })); 
  }
  
  protected void checkMixinNotUnique(MixinInfo.MixinMethodNode mixinMethod, SpecialMethod type) {
    if (this.mixin.isUnique())
      throw new InvalidMixinException(this.mixin, String.format("%s method %s found in a @Unique mixin %s", new Object[] { type, mixinMethod.name, this.mixin })); 
  }
  ...
  protected boolean attachUniqueMethod(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod) {
    ...
    if (this.strictUnique)
      throw new InvalidMixinException(this.mixin, String.format("Method conflict, %s method %s in %s cannot overwrite %s%s in %s", new Object[] { type, mixinMethod.name, this.mixin, target.name, target.desc, context
              .getTarget() })); 
    ...
    if (unique == null || !((Boolean)Annotations.getValue(unique, "silent", Boolean.FALSE)).booleanValue()) {
      if (Bytecode.hasFlag(mixinMethod, 64))
        try {
          ...
          logger.debug("Discarding sythetic bridge method {} in {} because existing method in {} is compatible", new Object[] { type, mixinMethod.name, this.mixin, context
                .getTarget() });
          return true;
      ...
      logger.warn("Discarding {} public method {} in {} because it already exists in {}", new Object[] { type, mixinMethod.name, this.mixin, context
            .getTarget() });
      return true;
    } 
    context.addMixinMethod(mixinMethod);
  ...
  protected void attachFields(MixinTargetContext context) {
    for (Iterator<FieldNode> iter = this.classNode.fields.iterator(); iter.hasNext(); ) {
      ...
      if (field.isUnique() && isShadow)
        throw new InvalidMixinException(this.mixin, String.format("@Shadow field %s cannot be @Unique", new Object[] { mixinField.name })); 
      ...
      if (target == null) {
        ...
        if (target == null)
          throw new InvalidMixinException(this.mixin, String.format("Shadow field %s was not located in the target class %s. %s%s", new Object[] { mixinField.name, context
                  .getTarget(), context.getReferenceMapper().getStatus(), 
                  getDynamicInfo(mixinField) })); 
        mixinField.name = field.renameTo(target.name);
      } 
      if (!Bytecode.compareFlags(mixinField, target, 8))
        throw new InvalidMixinException(this.mixin, String.format("STATIC modifier of @Shadow field %s in %s does not match the target", new Object[] { mixinField.name, this.mixin })); 
      if (field.isUnique()) {
        if ((mixinField.access & 0x6) != 0) {
        ...
        if (this.strictUnique)
          throw new InvalidMixinException(this.mixin, String.format("Field conflict, @Unique field %s in %s cannot overwrite %s%s in %s", new Object[] { mixinField.name, this.mixin, target.name, target.desc, context
                  .getTarget() })); 
        logger.warn("Discarding @Unique public field {} in {} because it already exists in {}. Note that declared FIELD INITIALISERS will NOT be removed!", new Object[] { mixinField.name, this.mixin, context
              .getTarget() });
        iter.remove();
        continue;
      } 
      if (!target.desc.equals(mixinField.desc))
        throw new InvalidMixinException(this.mixin, String.format("The field %s in the target class has a conflicting signature", new Object[] { mixinField.name })); 
      if (!target.name.equals(mixinField.name)) {
        if ((target.access & 0x2) == 0 && (target.access & 0x1000) == 0)
  ...
  protected boolean validateField(MixinTargetContext context, FieldNode field, AnnotationNode shadow) {
    if (Bytecode.hasFlag(field, 8) && 
      !Bytecode.hasFlag(field, 2) && 
      !Bytecode.hasFlag(field, 4096) && shadow == null)
      throw new InvalidMixinException(context, String.format("Mixin %s contains non-private static field %s:%s", new Object[] { context, field.name, field.desc })); 
    String prefix = (String)Annotations.getValue(shadow, "prefix", Shadow.class);
    if (field.name.startsWith(prefix))
      throw new InvalidMixinException(context, String.format("@Shadow field %s.%s has a shadow prefix. This is not allowed.", new Object[] { context, field.name })); 
    if ("super$".equals(field.name)) {
      if (field.access != 2)
        throw new InvalidMixinException(this.mixin, String.format("Imaginary super field %s.%s must be private and non-final", new Object[] { context, field.name })); 
      if (!field.desc.equals("L" + this.mixin.getClassRef() + ";"))
        throw new InvalidMixinException(this.mixin, 
            String.format("Imaginary super field %s.%s must have the same type as the parent mixin (%s)", new Object[] { context, field.name, this.mixin.getClassName() })); 
      return false;
    } 
    return true;
  }
  ...
  private static String getDynamicInfo(String targetType, AnnotationNode annotation) {
    ...
    return (description.length() > 0) ? String.format(" %s is @Dynamic(%s)", new Object[] { targetType, description }) : "";
  }
}
