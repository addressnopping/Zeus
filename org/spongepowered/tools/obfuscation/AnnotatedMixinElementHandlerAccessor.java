...
public class AnnotatedMixinElementHandlerAccessor extends AnnotatedMixinElementHandler implements IMixinContext {
  ...
  public void registerAccessor(AnnotatedElementAccessor elem) {
    if (elem.getAccessorType() == null) {
      elem.printMessage((Messager)this.ap, Diagnostic.Kind.WARNING, "Unsupported accessor type");
      return;
    } 
    String targetName = getAccessorTargetName(elem);
    if (targetName == null) {
      elem.printMessage((Messager)this.ap, Diagnostic.Kind.WARNING, "Cannot inflect accessor target name");
      return;
    } 
  ...
  private void registerAccessorForTarget(AnnotatedElementAccessor elem, TypeHandle target) {
    ...
    if (targetField == null) {
      if (!target.isImaginary()) {
        elem.printMessage((Messager)this.ap, Diagnostic.Kind.ERROR, "Could not locate @Accessor target " + elem + " in target " + target);
        return;
      } 
    ...
    if (obfData.isEmpty()) {
      ...
      elem.printMessage((Messager)this.ap, Diagnostic.Kind.WARNING, "Unable to locate obfuscation mapping" + info + " for @Accessor target " + elem);
      return;
    } 
    ...
    try {
      ...
      elem.printMessage((Messager)this.ap, Diagnostic.Kind.ERROR, "Mapping conflict for @Accessor target " + elem + ": " + ex.getNew() + " for target " + target + " conflicts with existing mapping " + ex
          .getOld());
    } 
  }
  ...
  private void registerInvokerForTarget(AnnotatedElementInvoker elem, TypeHandle target) {
    ...
    if (targetMethod == null) {
      if (!target.isImaginary()) {
        elem.printMessage((Messager)this.ap, Diagnostic.Kind.ERROR, "Could not locate @Invoker target " + elem + " in target " + target);
        return;
      } 
    ...
    try {
      ...
      elem.printMessage((Messager)this.ap, Diagnostic.Kind.ERROR, "Mapping conflict for @Invoker target " + elem + ": " + ex.getNew() + " for target " + target + " conflicts with existing mapping " + ex
          .getOld());
    } 
  }
